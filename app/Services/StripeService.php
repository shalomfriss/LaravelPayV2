...
class StripeService
{
    ...
    public function handleApproval()
    {
        if (session()->has('paymentIntentId')) {
            ...
            $paymentIntentObject = $this->makeRequest(
                'GET',
            ...
            if ($paymentIntentObject->status === 'requires_payment_method') {
                return redirect()
                    ->route('home')
            ...
            if ($confirmation->status === 'requires_action') {
                ...
                return Inertia::render('stripe/3d-secure', [
        ...
        return redirect()
            ->route('home')
            ->withErrors('We are unable to confirm your payment.  Please try again.');
    }
    ...
    public function handleSubscription(Request $request)
    {
        ...
        if ($paymentIntent->status === 'requires_action') {
            ...
            return Inertia::render('stripe/3d-secure-subscription', [
                'clientSecret' => $clientSecret,
                'plan' => $request->plan,
        ...
        return redirect()
            ...
            ->withErrors('We could not activate your subscription at this time.  Please try again');
    }
    ...
    protected function assertConfigured(?string $planSlug = null): void
    {
        ...
        if (blank($this->baseUri)) {
            $messages['payment_platform'] = 'Stripe base URI is not configured.';
        }

        if (blank($this->key) || blank($this->secret)) {
            $messages['payment_platform'] = 'Stripe credentials are not configured.';
        }

        if ($planSlug !== null && blank($this->plans[$planSlug] ?? null)) {
            $messages['plan'] = "Stripe price ID is not configured for [{$planSlug}].";
        }
